	...
	<div class="template-admin">
		<div class="admin-top">
			...
			<div class="admin-top-info">
				<h1>Новая работа в портфолио</h1>
			</div>
		</div>
		<div class="admin-content">
			<el-form @submit.native.prevent="onSubmit" :model="controls" :rules="rules" ref="form" class="demo-ruleForm">
				...
				<el-form-item class="text-center">
					...
					<el-button round native-type="submit" type="primary" :loading="loading">Добавить работу</el-button>
				</el-form-item>
			</el-form>
		</div>
	</div>
		...
		data() {
			return {
				...
				defaultTech: [
					...
					{value: 'Handlebars.js',label: 'Handlebars.js'},
					...
					if (valid && this.image) {
						...
						try {
							const formData = {
								...
								tech: this.controls.tech,
								date: this.controls.date,
								site_link: this.controls.site_link,
								repo_link: this.controls.repo_link,
								image: this.image
							}
							await this.$store.dispatch('work/create', formData)
							this.controls.name = ''
							this.controls.description = ''
							this.controls.tech = []
							this.controls.date = ''
							this.controls.site_link = ''
							this.controls.repo_link = ''
							this.image = []
							this.$refs.uploadImage.clearFiles()
							this.$message.success('Работа успешно добавлена')
							this.loading = false
						} catch (e) {
							this.loading = false
						}
